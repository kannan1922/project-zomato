...
function Navbar() {
  ...
  const handleOtp = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/signup/verify",
        postData
      );
      console.log(response.data.message);
      let temp: string = response.data.message;
  ...
  const handleOtpEmail = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/login/verify",
        postData
      );
      console.log(response.data.message);
      let temp: string = response.data.message;
  ...
  const handleOtpMobile = async () => {
    try {
      ...
      if (error.response.data.error === "Invalid OTP") setOtpVerify(true);
  ...
  const handleLogingoogle = async () => {
    try {
      const temp = window.location.assign(
        "http://localhost:4000/signup/google"
      );
  ...
  const fetchData = async (value: string) => {
    try {
      const response = await axios.get(
        `http://localhost:4000/search?startsWithLetter=${value}`
      );
      console.log("start");
          ...
          async (position) => {
            ...
            try {
              const response = await fetch(
                ...
                // AIzaSyDsklSLoF3zhP6Dif-NGQEQwiBkvwEwD74
                // AIzaSyDsklSLoF3zhP6Dif-NGQEQwiBkvwEwD74
              );
    ...
    <div className="zomato-navbar">
      <div className="logo-container">
        <img
          ...
          src="http://b.zmtcdn.com/web_assets/b40b97e677bc7b2ca77c58c61db266fe1603954218.png"
          alt="Zomato Logo"
        />
      </div>
      <div className="search-wrap">
        <div className="location-search">
          ...
          <input type="text" placeholder={`${locationName}`} />
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            width="15"
      ...
      {showOverlay && (
        <div className="overlay">
          {" "}
          <Search results={results.search} closeOverlay={toggleOverlay} />
        </div>
      )}
              ...
              {loading ? (
                ...
                <button
                  ...
                  className={`loginCreateAccount ${
                    !namecheck || !emailcheck || !agreeChecked ? "disabled" : ""
                  }`}
        ...
        {showOtp && !exist && (
          <div className="overlayOtp">
            ...
            <div className="otpText">
              Verification code has been sent to your email, a*****a@gmail.com,
            ...
            <div>
              ...
              <div className="timer">
                {timer > 0
                  ? `${Math.floor(timer / 60) < 10 ? "0" : ""}${Math.floor(
                      timer / 60
              ...
              <div className="resendOtp">
                ...
                <span
                  style={{
                    color: timer === 0 ? "red" : "rgba(.0 , .0, .0, 0.5)",
                    cursor: timer === 0 ? "pointer" : "inherit",
                  }}
      ...
      > */}
        ...
        {showOtpMobile && (
          <div className="overlayOtp">
            ...
            <div className="otpText">
              Verification code has been sent to Mobile Number, 7**********9,
              please enter the same here to complete the Login. Valid for 10
              minutes.
            </div>
            <div className="inputContainerOtp">
              <input
        ...
        {showEmail && (
          <div className="overlay1">
            ...
            <img
              ...
              src="http://b.zmtcdn.com/Zwebmolecules/73b3ee9d469601551f2a0952581510831595917292.png"
              alt=""
            />
            <div className="inputContainer">
              <input
        ...
        {exist && (
          <div className="overlayOtperror">
            ...
            <div className="responseError">{responseData}</div>
            <button onClick={handleOtperror} className="otpProceed">
              Try using other methods
            </button>
            <p className="responseErrorSkip" onClick={handleErrorSkip}>
              skip for now
            </p>
          </div>
        )}
